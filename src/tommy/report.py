...
.panel-head{{padding:17px 20px;border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;gap:14px}}.panel-head h2{{font-size:17px;margin:0}}.transcript-search{{min-width:0;width:220px;border:1px solid var(--line);border-radius:9px;padding:8px 10px;font:inherit}}.transcript{{height:calc(100vh - 130px);position:sticky;top:96px;display:flex;flex-direction:column}}.turns{{overflow:auto;padding:18px}}
...
<main class="layout"><section class="panel transcript"><div class="panel-head"><h2>Transcript</h2><input class="transcript-search" type="search" placeholder="Search {len(attempt["turns"])} turns" aria-label="Search transcript"></div><div class="turns">{turns}</div></section>
...
document.querySelector('.transcript-search').addEventListener('input',e=>{{const query=e.target.value.trim().toLowerCase();document.querySelectorAll('.turn').forEach(turn=>{{turn.hidden=query&&!turn.innerText.toLowerCase().includes(query)}})}});
</script></body></html>"""
